...
#define SHARDER_BUFSIZE (64 * 1024 * 1024)
    ...
    public:
        ...
        void start_preprocessing() {
            ...
            std::cout << sizeof(EdgeDataType) << " " << sizeof(edge_with_value<EdgeDataType>) << std::endl;
            curshovel_idx = 0;
            //std::cout  << "membudget_mb = " << get_option_int("membudget_mb", 1024) << " " << sizeof(get_option_int("membudget_mb", 1024)) << std::endl;
    ...
    protected:
        ...
        virtual void finish_shard(int shard, edge_t * shovelbuf, size_t shovelsize) {
            ...
            for(size_t i=0; i <= numedges; i++) {
                ...
                if ((edge.src != curvid) || edge.stopper()) {
                    ...
                    if (istart - last_index_output >= index_interval_edges) {
                        ...
                        size_t a = write(idxf, &sidx, sizeof(shard_index));  //write adjidx
                        assert(a>0);
            ...
            writea(f, buf, bufptr - buf);    // write adj
            free(buf);
            free(shovelbuf);
            close(f);
            close(idxf);
