    ...
    public:
        ...
        graphchi_engine(std::string _base_filename, int _nshards, bool _selective_scheduling, metrics &_m) : base_filename(_base_filename), nshards(_nshards), use_selective_scheduling(_selective_scheduling), m(_m) {
           ...
           /* logstream(LOG_INFO) << "Initializing graphchi_engine with dynamic edge-data. This engine expects " << sizeof(int)
            ...
            walk_manager->initialnizeWalks(nshards, (int)num_vertices(), base_filename ,intervals);
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        #pragma omp parallel for
                        for(int idx=0; idx <= (int)sub_interval_len; idx++) {
                                ...
                                if (exec_threads == 1 || v.parallel_safe) {
                                    ...
                                    if (v.scheduled) 
                                        /* --Rui */
                                        //userprogram.update(v, *walk_manager, chicontext);
                                        // userprogram.updateByVertex(v, *walk_manager, chicontext);
                                        userprogram.updateByWalk(vertices, vid, sub_interval_st, sub_interval_en, *walk_manager, chicontext);
                                }
                            }
                        }
        #pragma omp section
    ...
    public:
        ...
        void walkOnce(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, std::vector<svertex_t> &vertices){
            if (!is_inmemory_mode()) {
        ...
        void walksometimes(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, std::vector<svertex_t> &vertices, int t){
            int tt = t;
            do{
                if (!is_inmemory_mode()) {
                    exec_updates(userprogram, vertices);
                ...
                // logstream(LOG_INFO) << "walks in exec_interval : " << walk_manager.getWalksDis(exec_interval) << "  t:  " << tt << std::endl;
            }while( --tt );
        }

        void walktosomeextent(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, std::vector<svertex_t> &vertices, int t){
            int tt = t;
            do{
                if (!is_inmemory_mode()) {
                    exec_updates(userprogram, vertices);
                 ...
                 // logstream(LOG_INFO) << "walks in exec_interval : " << walk_manager.getWalksDis(exec_interval) << "  t:  " << tt << std::endl;
            }while( walk_manager->getWalksDis(exec_interval) > tt );
        }

        void walkToEnd(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, std::vector<svertex_t> &vertices){
            while(  walk_manager->notFinishInterval(exec_interval) ){
                if (!is_inmemory_mode()) {
                    exec_updates(userprogram, vertices);
                    /* Load phase after updates (used by the functional engine) */
                    load_after_updates(vertices);
                ...
                // logstream(LOG_INFO) << "walks in exec_interval : " << walk_manager.getWalksDis(exec_interval) << std::endl;
            }
        }

        void runInterval(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram){
                    metrics_entry mb = m.start_time();
                /* Determine interval limits */
                    ...
                    //--rui if (interval_st > interval_en) continue; // Can happen on very very small graphs.
                    if (interval_st > interval_en) return; //--Rui Can happen on very very small graphs.
                    ...
                    //m.stop_time(mb, "_before_exec_interval");
                    //while (sub_interval_st <= interval_en) {
                        ...
                        assert(sub_interval_en >= sub_interval_st);
                         ...
                         logstream(LOG_INFO) << "numIntervals : " << numIntervals++  <<"  exec_interval : " << exec_interval << std::endl;
                        ...
                        // walkOnce(userprogram, vertices);
                        walkToEnd(userprogram, vertices);
                        // walksometimes(userprogram, vertices, t);
                        // logstream(LOG_INFO) << "Finished updates" << std::endl;
                        
                        /* Save vertices */
                        metrics_entry ma = m.start_time();
                        if (!disable_vertexdata_storage) {
                        ...
                        //m.stop_time(mc, "_write-back-interval");
                    ...
                    m.stop_time(ma, "_after_exec_interval");
        }

        int NextInvlwithRandom( float c ){
            int cc = rand() % 100;
            if( cc < (int)(c*100) ){
                int p = walk_manager->intervalWithRandom();
                if( p != exec_interval ) return p;
            }
            return walk_manager->intervalWithMaxWalks();
        }

        int NextInvlwithMinStep( float c ){
            int cc = rand() % 100;
            if( cc < (int)(c*100)){
                int p = walk_manager->intervalWithMinStep();
                if( p != exec_interval ) return p;
            }
            return walk_manager->intervalWithMaxWalks();
        }

        void loadIteratively(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int niters ){
                ...
                for( int iter = 0; iter < niters; iter++ ){
                    for( int p = 0; p < nshards; p++){
                         //exec_interval = NextInvlwithRandom( prob );
                         exec_interval = p;
                        //exec_interval = walk_manager->intervalWithMaxWeight();
        ...
        void loadOnDemand(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, float prob){
                /* Keep the context object updated */
                chicontext.filename = base_filename;
                // chicontext.num_iterations = niters;
                chicontext.nvertices = num_vertices();
                ...
                while( walk_manager->notFinish() ){
                     //exec_interval = NextInvlwithRandom( prob );
                     exec_interval = NextInvlwithMinStep( prob );
                    //exec_interval = walk_manager->intervalWithMaxWeight();
                    walk_manager->printWalksDistribution( exec_interval );
                    metrics_entry mr = m.start_time();
                    runInterval(userprogram);
         ...
         */
        void run(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, float prob, int niters ) {
            ...
            loadOnDemand(userprogram, prob);
            // loadIteratively(userprogram, niters);
    ...
    public:
        ...
        /**
         * Replace all shards with zero values in vertices.
         */
        template<typename VT>
        void reinitialize_vertex_data(VT zerovalue) {
            std::string vdataname = filename_vertex_data<VertexDataType>(base_filename);
            int len = num_vertices() * sizeof(VT);
            // len = 1 * sizeof(VT);
            int f = open(vdataname.c_str(), O_RDWR );
            VT * buf =  (VT *) malloc(len);
            for(int i=0; i < (int) (len / sizeof(VT)); i++) {
                // std::cout << i << "   " << len <<  "   " << zerovalue << std::endl;
                buf[i] = zerovalue;
            }
            pwritea(f, buf, len, 0);
            close(f);
