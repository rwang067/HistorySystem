...
namespace graphchi {
    ...
    template <typename VertexDataType>
    std::vector<int > get_walk_distribution(std::string basefilename, int nshards ) {
        
        /* Initialize striped IO manager */
        metrics m("walk_distribution");
        stripedio * iomgr = new stripedio(m);
        ...
        /* Initialize buffer */
        int * shardsbuf = (int*) calloc(nshards, sizeof(int));
        ...
        while(st <= numvertices - 1) {
            ...
            /*load the intervals -- Rui*/
            std::vector<std::pair<vid_t, vid_t> > intervals;
            load_vertex_intervals(basefilename, nshards, intervals);
