    ...
    public:
        ...
        void walktosomeextent(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, std::vector<svertex_t> &vertices, int t){
            ...
            do{
                ...
                logstream(LOG_INFO) << "walks in exec_interval : " << walk_manager.getWalksDis(exec_interval) << "  t:  " << tt << std::endl;
            }while( iter > 0 && walk_manager.getWalksDis(exec_interval) > tt );
        }
        ...
        void walkToEnd(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, std::vector<svertex_t> &vertices){
            ...
            }while( iter > 0 && walk_manager.getWalksDis(exec_interval) > 0 );
        }

        void runInterval(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int t){
            /* Determine interval limits */
                    ...
                    while (sub_interval_st <= interval_en) {
                        ...
                        // walktosomeextent(userprogram, vertices, 100);
        ...
        void loadIteratively(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram){
            /* Main loop */
            for(iter=0; iter < niters; iter++) {
        ...
        void loadOnDemand(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int t){
                /* Keep the context object updated */
                chicontext.filename = base_filename;
                chicontext.num_iterations = niters;
                chicontext.nvertices = num_vertices();
                ...
                for(int interval_idx=0; interval_idx < nshards; ++interval_idx) {
                    exec_interval = interval_idx;
                    runInterval(userprogram, t);
                    walk_manager.printWalksDistribution( exec_interval );
                }
                /* -- move walks -- Rui */
                iter = 1;
                chicontext.iteration = iter;
                while( walk_manager.notEmpty() ){
         ...
         */
        void run(GraphChiProgram<VertexDataType, EdgeDataType, svertex_t> &userprogram, int _niters, int t) {
            ...
            /*main loop*/
            loadOnDemand(userprogram, t);
            // loadIteratively(userprogram);
