...
#define FLOATMAX 1000000

int toint(float a){
	return (randomInt(FLOATMAX)<(a)*FLOATMAX ? 1 : 0);
}
...
int main(int argc, char **argv) 
{
	std::cout<<"Format: /path/to/exe " 
		...
		<<"max_continuous_useless_blk ring_vert_count num_buffs iteration walks_per_source\n";
	...
	index_t walks_per_source = (index_t) atol(argv[15]);
...
#pragma omp barrier
			...
			if((tid & 1) == 0)
			{
				while(true)
				{	
					/*std::cout << "check process at line 291:\t"
						<< "tid=" << tid << "\t"
						<< "level=" << level << "\t"
						<< "it->is_bsp_done=" << it->is_bsp_done << "\t"
						<< std::endl;*/
					int chunk_id = -1;
					double blk_tm = wtime();
					...
					std::cout << "check process at line 318:\t"
						<< "tid=" << tid << "\t"
						<< "level=" << level << "\t"
						<< "chunk_id=" << chunk_id << "\t"
						<< "it->is_bsp_done=" << it->is_bsp_done << "\t"
					...
					while(true)
					{
						...
						index_t num_walks = (index_t) sa_curr[vert_id];
		                        ...
		                        for (int i = 0; i < num_walks; i++){
		                            vertex_t nextV;
		                            if (toint(0.85) && (end>beg)){
		                                nextV = pinst->buff[randomInt(end-beg)+beg];
		                            }
		                            else{
		                                nextV = randomInt(vert_count);
		                            }
		                            sa_next[nextV]++;
		                        }
		                        vertex_t nebr = pinst->buff[end-1];
