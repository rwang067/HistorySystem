...
struct AvgDegreeProgram : public GraphChiProgram<VertexDataType, EdgeDataType> {
...
public:
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType > &vertex, graphchi_context &gcontext) {
        if (gcontext.iteration == 0) {
            ...
            if (gcontext.iteration == L-1){
                avgdeg += 1.0/vertex.num_outedges();
            }
        }
    }
...
int main(int argc, const char ** argv) {
    ...
    program.avgdeg = R/program.avgdeg;
    std::cout << "Average degree : " << program.avgdeg << std::endl;

    float err = std::abs(program.avgdeg-13.5988)/13.5988;
